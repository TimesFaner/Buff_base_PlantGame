...
namespace Buff_system_timesfaner.TBuff_base
{
    ...
    public class BuffData : ScriptableObject
    {
        ...
        {
            var b = (BuffData)ScriptableObject.CreateInstance(buffName);
            b.Id = id;
            return b;
        }
        /// <summary>
        ...
        public void ClonetheTData(BuffInvoke T)
        {
            ...
            {
                var _type = T.GetType();
                var _SO = ScriptableObject.CreateInstance(_type) as BuffInvoke;
                _SO = Instantiate(T);
                _SO.name = T.name+"clone";
                OnCreate = _SO;
                AssetDatabase.CreateAsset(_SO, "Assets/Timesfaner_work/Buff_system_timesfaner/Data/Clone/"+_SO.name+".asset");
