...
namespace MoonSharp.Interpreter
{
    ...
    public static class ModuleRegister
    {
        ...
        {
            var moonsharp_table = DynValue.NewTable(table.OwnerScript);
        ...
        public static Table RegisterModuleType(this Table gtable, Type t)
        {
            ...
            foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                         _mi.IsStatic && _mi.GetCustomAttributes(typeof(MoonSharpModuleMethodAttribute), false)
                             .ToArray().Length > 0))
            {
            ...
            foreach (var fi in Framework.Do.GetFields(t).Where(_mi =>
                         _mi.IsStatic && _mi.GetCustomAttributes(typeof(MoonSharpModuleConstantAttribute), false)
                             .ToArray().Length > 0))
            {
