...
namespace MoonSharp.VsCodeDebugger.DebuggerLogic
{
    internal class MoonSharpDebugSession : DebugSession, IAsyncDebuggerClient
    {
        ...
        public override void Evaluate(Response response, Table args)
        {
            ...
            var v = m_Debug.Evaluate(expression) ?? DynValue.Nil;
            m_Variables.Add(v);
        ...
        private void ExecuteRepl(string cmd)
        {
            ...
            {
                var regex = cmd.Substring("seterror".Length).Trim();
            ...
            {
                var val = cmd.Substring("execendnotify".Length).Trim();
            ...
            {
                var currId = m_Server.CurrentId ?? -1000;

                foreach (var pair in m_Server.GetAttachedDebuggersByIdAndName())
                {
                    var isthis = pair.Key == m_Debug.Id ? " (this)" : "";
                    var isdef = pair.Key == currId ? " (default)" : "";
            ...
            {
                var arg = cmd.Substring("switch".Length).Trim();
        ...
        public override void SetBreakpoints(Response response, Table args)
        {
            ...
            var args_source = args["source"] as Table;

            if (args_source != null)
            {
                var p = args_source["path"].ToString();
            ...
            var src = m_Debug.FindSourceByName(path);
            ...
            var clientLines = args.Get("lines").Table;
            ...
            foreach (var l in lin) breakpoints.Add(new Breakpoint(lin2.Contains(l), l));
        ...
        {
            var maxLevels = getInt(args, "levels", 10);
            ...
            var level = 0;
            var max = Math.Min(maxLevels - 3, stack.Count);
            ...
            while (level < max)
            {
                ...
                var name = frame.Name;
                var sourceRef = frame.Location ?? DefaultSourceRef;
                var sourceIdx = sourceRef.SourceIdx;
                var path = sourceRef.IsClrLocation ? "(native)" : m_Debug.GetSourceFile(sourceIdx) ?? "???";
        ...
        {
            var threads = new List<Thread> { new(0, "Main Thread") };
        ...
        {
            var index = getInt(arguments, "variablesReference", -1);

            var variables = new List<Variable>();

            if (index == SCOPE_SELF)
            {
                var v = m_Debug.Evaluate("self");
