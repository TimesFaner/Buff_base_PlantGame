...
namespace MoonSharp.Interpreter
{
    ...
    public class Script : IScriptPrivateResource
    {
        ...
        {
            if (m_Debugger != null) m_Debugger.SetByteCode(m_ByteCode.Code.Select(s => s.ToString()).ToArray());
        }

        private void SignalSourceCodeChange(SourceCode source)
        {
            if (m_Debugger != null) m_Debugger.SetSourceCode(source);
        }
...
#pragma warning restore 618
            ...
            if (code is string) return LoadString((string)code, globalContext, friendlyFilename ?? filename);
        ...
        public static string GetBanner(string subproduct = null)
        {
            ...
            sb.AppendLine(string.Format("MoonSharp {0}{1} [{2}]", subproduct, VERSION,
