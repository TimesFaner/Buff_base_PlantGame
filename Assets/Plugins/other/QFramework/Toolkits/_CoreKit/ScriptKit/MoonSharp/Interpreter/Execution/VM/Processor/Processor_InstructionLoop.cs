...
namespace MoonSharp.Interpreter.Execution.VM
{
    internal sealed partial class Processor
    {
        ...
        private DynValue Processing_Loop(int instructionPtr)
        {
            ...
            var canAutoYield = AutoYieldCounter > 0 && m_CanYield && State != CoroutineState.Main;
            ...
            try
            {
                while (true)
                {
                    ...
                    if (m_Debug.DebuggerAttached != null) ListenDebugger(i, instructionPtr);
        ...
        private int ExecIndex(Instruction i, int instructionPtr)
        {
            ...
            while (nestedMetaOps > 0)
            {
                ...
                else if (obj.Type == DataType.UserData)
                {
                    ...
                    if (v == null) throw ScriptRuntimeException.UserDataMissingField(ud.Descriptor.Name, idx.String);
