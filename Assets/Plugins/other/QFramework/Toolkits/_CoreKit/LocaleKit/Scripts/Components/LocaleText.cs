...
namespace QFramework
{
    public class LocaleText : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (SetTextOnInit) UpdateText(LocaleKit.CurrentLanguage);
        }

        private void UpdateText(Language language)
        {
            if (mText) mText.text = LanguageTexts.First(lt => lt.Language == language).Text;

            if (mTextMesh) mTextMesh.text = LanguageTexts.First(lt => lt.Language == language).Text;
        }
    }
