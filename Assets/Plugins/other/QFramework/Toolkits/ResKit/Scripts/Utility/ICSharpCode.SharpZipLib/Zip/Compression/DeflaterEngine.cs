...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
    ...
    public class DeflaterEngine
    {
        ...
        public void Reset()
        {
            ...
            for (var i = 0; i < DeflaterConstants.HASH_SIZE; i++) head[i] = 0;

            for (var i = 0; i < DeflaterConstants.WSIZE; i++) prev[i] = 0;
        }
        ...
        private int InsertString()
        {
            short match;
            var hash =
                ((ins_h << DeflaterConstants.HASH_SHIFT) ^ window[strstart + (DeflaterConstants.MIN_MATCH - 1)]) &
