...
namespace ICSharpCode.SharpZipLib.BZip2
{
    ...
    public class BZip2OutputStream : Stream
    {
        ...
        private void SendMTFValues()
        {
            ...
            for (var i = 0; i < BZip2Constants.GroupCount; ++i) len[i] = new char[BZip2Constants.MaximumAlphaSize];
            ...
            for (var i = 0; i < BZip2Constants.GroupCount; ++i) rfreq[i] = new int[BZip2Constants.MaximumAlphaSize];
            ...
            if (!(nSelectors < 32768 && nSelectors <= 2 + 900000 / BZip2Constants.GroupSize)) Panic();
            ...
            for (var i = 0; i < BZip2Constants.GroupCount; ++i) code[i] = new int[BZip2Constants.MaximumAlphaSize];
        ...
        private void MainSort()
        {
            ...
            for (i = 0; i < BZip2Constants.OvershootBytes; i++) block[last + i + 2] = block[i % (last + 1) + 1];
            for (i = 0; i <= last + BZip2Constants.OvershootBytes; i++) quadrant[i] = 0;
                ...
                for (i = 0; i <= 255; i++)
                {
                    ...
                    if (i < 255)
                    {
                        ...
                        for (j = 0; j < bbSize; j++)
                        {
                            ...
                            if (a2update < BZip2Constants.OvershootBytes) quadrant[a2update + last + 1] = qVal;
                        }
        ...
        private static void HbMakeCodeLengths(char[] len, int[] freq, int alphaSize, int maxLen)
        {
            ...
            while (true)
            {
                ...
                if (!(nHeap < BZip2Constants.MaximumAlphaSize + 2)) Panic();
                ...
                while (nHeap > 1)
                {
                    ...
                    weight[nNodes] = (int)((weight[n1] & 0xffffff00) + (weight[n2] & 0xffffff00)) |
                                     (1 + ((weight[n1] & 0x000000ff) > (weight[n2] & 0x000000ff)
                ...
                if (!(nNodes < BZip2Constants.MaximumAlphaSize * 2)) Panic();
