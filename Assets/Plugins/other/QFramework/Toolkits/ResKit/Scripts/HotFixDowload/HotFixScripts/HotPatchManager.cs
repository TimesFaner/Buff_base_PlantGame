...
namespace QFramework
{
    public class HotPatchManager : ISingleton
    {
        ...
        private IEnumerator UnPackToPersistentDataPath(Action callBack)
        {
            ...
            {
                var unityWebRequest = UnityWebRequest.Get(Application.streamingAssetsPath + "/" + fileName);
                ...
                if (m_PackedMd5.ContainsKey(fileName)) AlreadyUnPackSize += m_PackedMd5[fileName].ABSize;
        ...
        private bool CheckLocalAndServerPatch()
        {
            ...
            if (localGameVesion != null && m_GameVersion.Pathces != null && localGameVesion.Pathces != null &&
                m_GameVersion.Pathces.Length > 0 && m_GameVersion.Pathces[m_GameVersion.Pathces.Length - 1].Version !=
                localGameVesion.Pathces[localGameVesion.Pathces.Length - 1].Version)
                return true;
        ...
        {
            var versionTex = Resources.Load<TextAsset>("Version");
            if (versionTex == null)
            {
            ...
            var all = versionTex.text.Split('\n');
            if (all.Length > 0)
            {
                var infoList = all[0].Split(';');
        ...
        {
            var xmlUrl = "http://annuzhiting2.oss-cn-hangzhou.aliyuncs.com/ServerInfo.xml";
        ...
        private void GetHotAB()
        {
            ...
            {
                var lastPatches = m_GameVersion.Pathces[m_GameVersion.Pathces.Length - 1];
                if (lastPatches != null && lastPatches.Files != null)
                    foreach (var patch in lastPatches.Files)
                    ...
                    foreach (var patch in CurrentPatches.Files)
                        if ((Application.platform == RuntimePlatform.WindowsPlayer ||
                            ...
                            AddDownLoadList(patch);
                                  ...
                                  Application.platform == RuntimePlatform.WindowsEditor) &&
                                 patch.Platform.Contains("IOS")) AddDownLoadList(patch);
            }
        }

        private void AddDownLoadList(Patch patch)
        {
            var filePath = m_DownLoadPath + "/" + patch.Name;
            //存在这个文件时进行对比看是否与服务器MD5码一致，不一致放到下载队列，如果不存在直接放入下载队列
            if (File.Exists(filePath))
            {
                var md5 = MD5Manager.Instance.BuildFileMd5(filePath);
                Debug.Log(filePath);
                if (patch.Md5 != md5)
                {
        ...
        {
            var alreadySize = m_AlreadyDownList.Sum(x => x.Size);
            float curAlreadySize = 0;
            if (m_CurDownload != null)
            {
                var patch = FindPatchByGamePath(m_CurDownload.FileName);
        ...
        public IEnumerator StartDownLoadAB(Action callBack, List<Patch> allPatch = null)
        {
            ...
            var downLoadAssetBundles = new List<DownLoadAssetBundle>();
            ...
            foreach (var downLoad in downLoadAssetBundles)
            {
                ...
                var patch = FindPatchByGamePath(downLoad.FileName);
        ...
        {
            var downLoadList = new List<Patch>();
            ...
            else
            {
                if (m_TryDownCount >= DOWNLOADCOUNT)
                {
                    var allName = "";
                    StartDownload = false;
                    foreach (var patch in downLoadList) allName += patch.Name + ";";
                ...
                else
                {
                    m_TryDownCount++;
                    m_DownLoadMD5Dic.Clear();
                    foreach (var patch in downLoadList) m_DownLoadMD5Dic.Add(patch.Name, patch.Md5);
                    //自动重新下载校验失败的文件
