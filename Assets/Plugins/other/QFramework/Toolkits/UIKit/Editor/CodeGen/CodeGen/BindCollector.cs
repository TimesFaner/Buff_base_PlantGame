...
namespace QFramework
{
    public class BindCollector
    {
        ...
        public static void SearchBinds(Transform curTrans, string transFullName, PanelCodeInfo panelCodeInfo = null,
            ElementCodeInfo parentElementCodeInfo = null, Type leafPanelType = null)
        {
            foreach (Transform childTrans in curTrans)
            {
                ...
                if (null != uiMark)
                {
                    ...
                    {
                        if (!panelCodeInfo.BindInfos.Any(markedObjInfo =>
                                markedObjInfo.TypeName.Equals(uiMark.Transform.name)))
                        {
                    ...
                    {
                        if (!parentElementCodeInfo.BindInfos.Any(markedObjInfo =>
                                markedObjInfo.TypeName.Equals(uiMark.Transform.name)))
                        {
                            ...
                            parentElementCodeInfo.DicNameToFullName.Add(uiMark.Transform.name,
