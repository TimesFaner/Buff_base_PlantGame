...
namespace Mirror.Examples.LagCompensationDemo
{
    public class ServerCube : MonoBehaviour
    {
        ...
        public bool CmdClicked(Vector2 position)
        {
            ...
            Debug.Log(
                $"CmdClicked: serverTime={NetworkTime.localTime:F3} clientTime={client.localTimeline:F3} estimatedTime={estimatedTime:F3} estimationError={error:F3} position={position}");
            ...
            if (LagCompensation.Sample(history, estimatedTime, lagCompensationSettings.captureInterval,
                    out resultBefore, out resultAfter, out var t))
            {
